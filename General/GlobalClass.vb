...
Public Class GlobalClass
    ...
    Sub ProcessJob(jobPath As String)
        Try
            ...
            If p.MTN Then g.DefaultCommands.ExecuteCommandLine(Package.Items("MTNWindows").Path.Escape + " " + p.TargetFile, False, True, False)
            Dim Obtain_Length = MediaInfo.GetGeneral(p.TargetFile, "Duration").ToInt
            If Obtain_Length <= 10 And Obtain_Length > 5 Then
                If p.GIF Then g.DefaultCommands.ExecuteCommandLine(Package.Items("ffmpeg").Path.Escape + " -ss 300.0 -t 4.2 -i %target_files% -vf ""fps=15,scale=480:-1:flags=spline,palettegen=stats_mode=diff"" -loglevel quiet -y ""%target_temp_file%.png"" || exit" + BR + Package.Items("ffmpeg").Path.Escape + " -ss 25.0 -t 4.2 -i %target_files% -i ""%target_temp_file%.png"" -lavfi ""fps=15,scale=480:-1:flags=spline [x]; [x][1:v] paletteuse=dither=floyd_steinberg"" -loglevel quiet -y ""%target_dir%%target_name%.gif""", True, True, True)
            ElseIf Obtain_Length > 30 Then
                If p.GIF Then g.DefaultCommands.ExecuteCommandLine(Package.Items("ffmpeg").Path.Escape + " -ss 900.0 -t 4.2 -i %target_files% -vf ""fps=15,scale=480:-1:flags=spline,palettegen=stats_mode=diff"" -loglevel quiet -y ""%target_temp_file%.png"" || exit" + BR + Package.Items("ffmpeg").Path.Escape + " -ss 25.0 -t 4.2 -i %target_files% -i ""%target_temp_file%.png"" -lavfi ""fps=15,scale=480:-1:flags=spline [x]; [x][1:v] paletteuse=dither=floyd_steinberg"" -loglevel quiet -y ""%target_dir%%target_name%.gif""", True, True, True)
            ElseIf Obtain_Length <= 5 Then
