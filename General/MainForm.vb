...
#End Region
    ...
    Sub ShowOptionsDialog(pagePath As String)
        Using form As New SimpleSettingsForm(
            ...
            StaxRipThumbnailOption.Text = "Enable StaxRip Thumbnail Creation"
            StaxRipThumbnailOption.Help = "Saves thumbnails in the target folder. Customizations can be made in the settings under:" + BR2 + "General > Advanced > Thumbnails"
            StaxRipThumbnailOption.Field = NameOf(p.SaveThumbnails)


            MTNThumbnailOption.Text = "Enable MTN Thumbnail Creation"
            ...
            Animated.Text = "Gif Creation"
            Animated.Help = "Picks a Random Spot and creates a 2 to 5 Second Animation."
            Animated.Field = NameOf(p.GIF)

            AnimatedPNG.Text = "PNG Creation"
            AnimatedPNG.Help = "Picks a Random Spot and creates a 2 to 5 Second Animation."
            ...
            b = ui.AddBool()
            b.Text = "HDR Metadata"
            b.Help = "Adds the Remaining Metadata Required for HDR10 for MKV Files"
    ...
    Shared Function GetDefaultMenuMain() As CustomMenuItem
        ...
        ret.Add("Tools|Advanced|Check For Update", NameOf(CheckingforUpdates))
        ...
        ret.Add("Apps|MKVInfo", NameOf(MKVInfoSelect))
        ...
        ret.Add("Apps|Gifs|Animated GIF", NameOf(GifCreation))
        ret.Add("Apps|Gifs|Animated PNG", NameOf(PNGAnimation))
    ...
    Sub CheckingforUpdates()
        ...
        MsgOK("Application Will Shutdown to Check for Updates")
            ...
            If fd.ShowDialog = DialogResult.OK Then
                Using f As New SimpleSettingsForm("Thumbnail Options")
                    ...
                    If f.ShowDialog() = DialogResult.OK Then
                        ...
                        If fd.Multiselect = False Then

                            MsgOK("Thumbnail Complete")
                        End If
                    End If
                End Using
            End If
    ...
    <Command("Uncompressed Animations")>
        Sub PNGAnimation()
            Using fd As New OpenFileDialog
                fd.Title = "Select files"
                ...
                If fd.ShowDialog = DialogResult.OK Then
                    Using f As New SimpleSettingsForm("PNG Options")
                        ...
                        Dim RateDen = ui.AddNum()
                        RateDen.Text = "FrameRate:"
                        RateDen.Config = {0, 60}
                        RateDen.NumEdit.Value = s.Storage.GetInt("RateDen", 15)
                        RateDen.NumEdit.SaveAction = Sub(value) s.Storage.SetInt("RateDen", CInt(value))

                        'Due to Scripting, Software Can only Seek through Frames or Selected Range.
                        Dim FrameStart = ui.AddNum()
                        FrameStart.Text = "Starting Frame:"
                        FrameStart.Config = {0, 9999}
                        FrameStart.NumEdit.Value = s.Storage.GetInt("FrameStart", 719)
                        FrameStart.NumEdit.SaveAction = Sub(value) s.Storage.SetInt("FrameStart", CInt(value))

                        'Same As Above.  
                        Dim FrameEnd = ui.AddNum()
                        FrameEnd.Text = "Ending Frame:"
                        FrameEnd.Config = {0, 9999}
                        FrameEnd.NumEdit.Value = s.Storage.GetInt("FrameEnd", 899)
                        FrameEnd.NumEdit.SaveAction = Sub(value) s.Storage.SetInt("FrameEnd", CInt(value))

                        Dim Height = ui.AddNum()
                        Height.Text = "Height:"
                        Height.Config = {240, 1920}
                        Height.NumEdit.Value = s.Storage.GetInt("PNGHeight", 480)
                        Height.NumEdit.SaveAction = Sub(value) s.Storage.SetInt("PNGHeight", CInt(value))

                        Dim Width = ui.AddNum()
                        Width.Text = "Width:"
                        Width.Config = {240, 1080}
                        Width.NumEdit.Value = s.Storage.GetInt("PNGWidth", 270)
                        Width.NumEdit.SaveAction = Sub(value) s.Storage.SetInt("PNGWidth", CInt(value))
                        ...
                        If f.ShowDialog() = DialogResult.OK Then
                            ...
                            For Each i In fd.FileNames
                                Try
                                    VDub2PNG.PNG(i, Nothing)
                                Catch ex As Exception
                                    g.ShowException(ex)
                                End Try
                            Next
                        End If
                        MsgOK("Animation Complete")
