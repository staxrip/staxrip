Public Class Strings
    ...
    Public Const ProjectX As String = "ProjectX demuxes and fixes MPEG-2 TS, H264/AVC and H265/HEVC are not supported. ProjectX can be enabled or disabled in the settings on the Demuxing tab. ProjectX requires Java x64 or x86."
    Public Const dsmux As String = "dsmux is installed by the Haali Splitter and is used to mux TS containing AVC into MKV. dsmux causes less async problems then mkvmerge." + CrLf2 + "dsmux can be enabled or disabled in the settings on the Demuxing tab. LAV Filters and Haali Splitter overrite each other, most people prefer LAV Filters, therefore it's recommended to install Haali first and LAV Filters last."
    ...
    Public Const DGDecNV As String = "DGDecNV is a shareware AviSynth source filter using NVIDIA GPU powered hardware acceleration. DGIndexNV can be enabled and configured at Tools/Setting/Demux. Which file types DGIndexNV handles can be configured. DGIndexNV can demux audio with proper av sync."
    ...
    Public Const DGAVCIndex As String = "StaxRip x64 uses DGAVCIndex only for demuxing because the AviSynth source filter isn't available for x64. DGAVCIndex can be enabled at Tools/Settings/Demuxing. Which file types DGAVCIndex handles can be configured. DGAVCIndex can process TS keeping audio in sync, an alternative method to process AVC TS is using dsmux from Haali splitter which converts TS to MKV, it does this with better AV sync then mkvmerge."
