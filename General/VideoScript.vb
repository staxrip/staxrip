...
"
    ...
    Shared Function GetDefaults() As List(Of TargetVideoScript)
        ...
        script.Filters.Add(New VideoFilter("Resize", "Spline36Resize", "clip = core.resize.Spline36(clip, %target_width%, %target_height%)", False))
        ret.Add(script)
...
Public Class FilterCategory
    ...
    Shared Function GetVapourSynthDefaults() As List(Of FilterCategory)
        ...
        Dim color As New FilterCategory("Color")
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Gamma to Linear", "clip = Dither.gamma_to_linear(clip, curve=""$select:msg:Select the Color Curve;601;709;2020$"")"))
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Linear to Gamma", "clip = Dither.linear_to_gamma(clip, curve=""$select:msg:Select the Color Curve;601;709;2020$"")"))
        ...
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Stack | Native to Stack16", "clip = core.fmtc.resample (clip, css=""444"")" + BR + "clip = core.fmtc.nativetostack16(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Stack | Stack16 to Native", "clip = fmtc.stack16tonative(clip)"))
        ...
        color.Filters.Add(New VideoFilter(color.Name, "Convert | To RGB", "clip = mvsfunc.ToRGB(clip,matrix=""$select:msg:Select Matrix;470bg;240;709;2020;2020cl;bt2020c$"")"))
        color.Filters.Add(New VideoFilter(color.Name, "Convert | To YUV", "clip = mvsfunc.ToYUV(clip,matrix=""$select:msg:Select Matrix;470bg;240;709;2020;2020cl;bt2020c$"")"))
        ret.Add(color)
        ...
        Dim resize As New FilterCategory("Resize")
        resize.Filters.Add(New VideoFilter(resize.Name, "Resize | Resize", "clip = core.resize.$select:Bilinear;Bicubic;Point;Lanczos;Spline16;Spline36$(clip, %target_width%, %target_height%)"))
        resize.Filters.Add(New VideoFilter(resize.Name, "Resize | ReSample", "clip = core.fmtc.resample(clip, w=%target_width%, h=%target_height%)"))
        resize.Filters.Add(New VideoFilter(resize.Name, "Dither_Resize16", "clip = Dither.Resize16nr(clip, %target_width%, %target_height%)"))
        ret.Add(resize)
        ...
        restoration.Filters.Add(New VideoFilter(restoration.Name, "DeHalo | abcxyz", "clip = muvsfunc.abcxyz(clip)"))
        restoration.Filters.Add(New VideoFilter(restoration.Name, "DeHalo | YAHR | YAHR", "clip = havsfunc.YAHR(clip)"))
        restoration.Filters.Add(New VideoFilter(restoration.Name, "DeHalo | YAHR | YAHRmod", "clip = muvsfunc.YAHRmod(clip)"))
        ...
        restoration.Filters.Add(New VideoFilter(restoration.Name, "RCR | Vinverse", "clip = havsfunc.Vinverse(clip)"))
        restoration.Filters.Add(New VideoFilter(restoration.Name, "RCR | Vinverse2", "clip = havsfunc.Vinverse2(clip)"))
        ret.Add(restoration)
