...
Public Class FilterCategory
    ...
    Shared Function GetAviSynthDefaults() As List(Of FilterCategory)
        ...
        ''color.Filters.Add(New VideoFilter(color.Name, "HDRCore | Bitdepth", "Bitdepth(from=$select:msg:Select Input BitDepth;8;16;32;88$, to=$select:msg:Select Output BitDepth;8;16;32;88$)"))
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Levels | Levels (16-235 to 0-255)", "Levels(16, 1, 235, 0, 255, coring=False)"))
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Levels | Levels (0-255 to 16-235)", "Levels(0, 1, 255, 16, 235, coring=False)"))
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Stack | ToStack", "ConvertToStacked()"))
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Stack | FromStack", "ConvertFromStacked()"))
    ...
    Shared Function GetVapourSynthDefaults() As List(Of FilterCategory)
        ...
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Sigmoid Inverse", "clip = havsfunc.SigmoidInverse(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "Dither | Sigmoid Direct", "clip = havsfunc.SigmoidDirect(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "Dither | SmoothGrad", "clip = fvsfunc.GradFun3(src=clip, mode=6, smode=1)"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | ColorSpace | Transfer", "clip = fmtc.transfer(clip, transs=""$select:msg:Select Input;Linear;470m;470bg;240m;SRGB;709;2020;2084$"", transd=""$select:msg:Select Output;Linear;470m;470bg;240m;SRGB;709;2020;2084$"")"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | Native to Stack16", "clip = fmtc.nativetostack16(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | Stack16 to Native", "clip = fmtc.stack16tonative(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | ColorSpace | BitDepth", "clip = core.fmtc.bitdepth (clip, bits=$select:msg:Select BitDepth;8;10;12;16;32$)"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | ColorSpace | Matrix | Matrix", "clip = fmtc.matrix(clip, mat=""$select:msg:Select Matrix;Linear;470m;470bg;240m;SRGB;709;2020$"")"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | ColorSpace | Matrix | Matrix 2020", "fmtc.matrix2020cl(clip)"))
        color.Filters.Add(New VideoFilter(color.Name, "FMTC | ColorSpace | Prime", "clip = fmtc.primaries(clip, prims=""$select:msg:Select Input;470m;470bg;240m;SRGB;709;2020$"", primd=""$select:msg:Select Output;Linear;470m;470bg;240m;SRGB;709;2020$"")"))
        color.Filters.Add(New VideoFilter(color.Name, "Interlaced Bobbing", "clip = core.std.SeparateFields(clip, tff=True)" + BR + "clip = core.fmtc.resample (clip, scalev=2, kernel=""cubic"", interlaced=True, interlacedd=False)"))
        color.Filters.Add(New VideoFilter(color.Name, "", ""))

        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Tweak", "clip = adjust.Tweak(clip, sat=0.75, hue=105-138)"))
        ...
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | Range", "$select:msg:Select Range;PC to TV|clip = core.fmtc.bitdepth (clip, fulls=True, fulld=False);TV to PC|clip = core.fmtc.bitdepth (clip=c, fulls=False, fulld=True)$"))
        ...
        color.Filters.Add(New VideoFilter(color.Name, "Convert | RGB to YUV", "clip = core.fmtc.resample (clip, css=""444"")" + BR + "clip = core.fmtc.matrix (clip, mat=""709"", col_fam=vs.YUV, bits=16)" + BR + "clip = core.fmtc.resample (clip, css=""420"")" + BR + "clip = core.fmtc.bitdepth (clip, bits=8)"))
        ...
        color.Filters.Add(New VideoFilter(color.Name, "ColorYUV | DVD to HD", "clip = core.fmtc.resample (clip, css=""444"")" + BR + "clip = core.fmtc.matrix (clip, mats=""601"", matd=""709"")" + BR + "clip = core.fmtc.resample (clip, css=""420"")" + BR + "clip = core.fmtc.bitdepth (clip, bits=8)"))
        ret.Add(color)
